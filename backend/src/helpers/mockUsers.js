...
const adminUser = {
  firstName: "Abdullah",
  lastName: "Garip",
  email: "garip@gmail.com",
  password: "Garip123*",
  contactNumber: "05551112347",
  image: "https://cdn.pixabay.com/photo/2025/04/24/22/36/beach-9556784_640.jpg",
  role: 1,
};
...
const createFakeUsers = async (count = 100) => {
  try {
    ...
    const adminHashedPassword = passwordEncrypt(adminUser.password, adminSalt);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const user = {
        ...
        role: faker.number.int({ min: 2, max: 3 }),
      ...
      for (const user of usersWithoutSalt) {
        if (!user.password || user.password.length > 32) {
          console.error(
            `Skipping user ${user._id} - invalid or already hashed password.`
          );
          continue;
        }
      }
